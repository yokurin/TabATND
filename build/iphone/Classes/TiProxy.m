...
//Rare exceptions to indicate a bug in the _tabatnd code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_tabatnd = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_tabatnd]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.tabatnd.module"
